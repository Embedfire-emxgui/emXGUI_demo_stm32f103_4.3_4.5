...
void VS_Init(void)
{
  ...
  GPIO_SetBits(VS_GPIO_HORN_PORT, VS_HORN);    // 默认禁用喇叭
...
u8 VS_HD_Reset(void)
{
	...
	VS_XCS_SET;//取消数据传输
	VS_RST_SET;	   
	while(VS_DREQ_IN==0&&retry<20)//等待DREQ为高
	{
		retry++;
		GUI_msleep(1);
	};
...
/***************喇叭设置****************/
//喇叭选择 DISABLE：禁用 ENABLE：启用
void VS_Horn_Set(FunctionalState State)
{
  if (State == DISABLE)
  {
    GPIO_SetBits(VS_GPIO_HORN_PORT, VS_HORN);    // 禁用喇叭
  }
  else if (State == ENABLE)
  {
    GPIO_ResetBits(VS_GPIO_HORN_PORT, VS_HORN);    // 启用禁用喇叭
  }
}
