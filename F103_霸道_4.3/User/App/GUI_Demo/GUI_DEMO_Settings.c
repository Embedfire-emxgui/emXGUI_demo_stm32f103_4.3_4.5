...
const set_icon_t set_icon[] = {
  ...
  {L"设置",        {50, 0,  220,  40}, ID_SETTINGS_TITLE},      // 3. 
  {L"主题",        {10, 66, 100, 23}, ID_SETTINGS_THEMEINFO},   // 4. 
};
...
static void theme_button_OwnerDraw(DRAWITEM_HDR *ds)
{
  ...
  DrawText(hdc_temp, wbuf, -1, &rc_tmp1, DT_TOP|DT_CENTER);//绘制文字(居中对齐方式)
   ...
   switch(msg){
      case WM_CREATE:
      {
          CreateWindow(BUTTON, L"F", WS_OWNERDRAW | WS_VISIBLE,
                        10, 7, 30, 30, hwnd, ID_DET_EXIT, NULL, NULL); 
      ...
      case WM_PAINT:
      {
        ...
        DrawText(hdc, L"512KB+8MB", -1, &rc, DT_VCENTER|DT_RIGHT);//绘制文字(居中对齐方式) 
        HLine(hdc, rc.x, rc.y+rc.h, rc.x+310);
...
{ 
   switch(msg){
      case WM_CREATE:
      {
        ...
        if (strstr(GUI_SETTINGS_BACKGROUNG_PIC, "0:/") != NULL)
        {
