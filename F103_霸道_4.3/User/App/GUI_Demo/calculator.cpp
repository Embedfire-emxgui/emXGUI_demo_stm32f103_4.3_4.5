...
extern "C" int	Calculator_WinMain(void)
{
  ...
  output = (char *)GUI_VMEM_Alloc(MAXLEN * sizeof(char));
  input = (char *)GUI_VMEM_Alloc(MAXLEN * sizeof(char));
  ident = (iden *)GUI_VMEM_Alloc(MAXIDENT * sizeof(iden));
  hisexp = (char (*)[MAXLEN])GUI_VMEM_Alloc(MAXLEN * MAXLEN * sizeof(char));
  stack = (char (*)[MAXLEN])GUI_VMEM_Alloc(MAXLEN * MAXLEN * sizeof(char));
  wstr_buf = (WCHAR *)GUI_VMEM_Alloc(1024 * sizeof(WCHAR));
  str_buf = (char *)GUI_VMEM_Alloc(1024 * sizeof(char));
  ...
  GUI_VMEM_Free(output);
  GUI_VMEM_Free(input);
  GUI_VMEM_Free(ident);
  GUI_VMEM_Free(hisexp);
  GUI_VMEM_Free(stack);
  GUI_VMEM_Free(wstr_buf);
  GUI_VMEM_Free(str_buf);
