...
{	
	write_cmd ( CMD_SetCoordinateX ); 				 /* 设置X坐标 */
	write_dat ( usX >> 8  );	 /* 先高8位，然后低8位 */
//  write_cmd ( CMD_SetCoordinateX + 1 ); 
	write_dat ( usX & 0xff  );	 /* 设置起始点和结束点*/
//  write_cmd ( CMD_SetCoordinateX + 2 );
	write_dat ( ( usX + usWidth - 1 ) >> 8  );
//  write_cmd ( CMD_SetCoordinateX + 3 );
	write_dat ( ( usX + usWidth - 1 ) & 0xff  );

	write_cmd ( CMD_SetCoordinateY ); 			     /* 设置Y坐标*/
	write_dat ( usY >> 8  );
//  write_cmd ( CMD_SetCoordinateY + 1);
	write_dat ( usY & 0xff  );
//  write_cmd ( CMD_SetCoordinateY + 2);
	write_dat ( ( usY + usHeight - 1 ) >> 8 );
//  write_cmd ( CMD_SetCoordinateY + 3);
	write_dat ( ( usY + usHeight - 1) & 0xff );
