...
/*定义远端IP信息*/
uint8  remote_ip[4]={192,168,0,3};											/*远端IP地址*/
...
void gpio_for_w5500_config(void)
{
	...
	RCC_APB2PeriphClockCmd(WIZ_GPIO_CLK|WIZ_RESET_INT_CLK, ENABLE);
	/*初始化STM32 SPI2接口*/
	RCC_APB1PeriphClockCmd(WIZ_SPI_CLK, ENABLE);
	
	GPIO_InitStructure.GPIO_Pin = WIZ_SCLK | WIZ_MISO | WIZ_MOSI;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;		   // 复用输出
	GPIO_Init(WIZ_GPIO_PORT, &GPIO_InitStructure);
   
   GPIO_InitStructure.GPIO_Pin = WIZ_SCS;
   //GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
   GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;	   // 推免输出
   GPIO_Init(WIZ_RESET_INT_PORT, &GPIO_InitStructure);						  
...
void wiz_cs(uint8_t val)
{
	if (val == LOW) 
	{
		GPIO_ResetBits(WIZ_RESET_INT_PORT, WIZ_SCS); 
	}
	else if (val == HIGH)
	{
		GPIO_SetBits(WIZ_RESET_INT_PORT, WIZ_SCS); 
	}
}
...
unsigned char SPI2_ReadWrite(unsigned char writedat)
{
	...
	SPI_I2S_SendData(WIZ_SPI, writedat);
