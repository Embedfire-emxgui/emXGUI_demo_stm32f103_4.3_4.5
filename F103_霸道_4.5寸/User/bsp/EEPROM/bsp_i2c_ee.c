...
{
	if (i2c_ee_CheckDevice(macEE_DEV_ADDR) == 0)
	{
		return 1;
	}
	else
...
uint8_t ee_ReadBytes(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
	...
	i2c_ee_SendByte(macEE_DEV_ADDR | macI2C_EE_WR);	/* 此处是写指令 */
	...
	i2c_ee_SendByte((uint8_t)_usAddress);
	...
	i2c_ee_SendByte(macEE_DEV_ADDR | macI2C_EE_RD);	/* 此处是读指令 */
	...
	for (i = 0; i < _usSize; i++)
	{
		_pReadBuf[i] = i2c_ee_ReadByte();	/* 读1个字节 */
		
		/* 每读完1个字节后，需要发送Ack， 最后一个字节不需要Ack，发Nack */
		if (i != _usSize - 1)
		{
			i2c_ee_Ack();	/* 中间字节读完后，CPU产生ACK信号(驱动SDA = 0) */
		}
		else
		{
			i2c_ee_NAck();	/* 最后1个字节读完后，CPU产生NACK信号(驱动SDA = 1) */
		}
	}
	/* 发送I2C总线停止信号 */
	i2c_ee_Stop();
	...
	for (i = 0; i < _usSize; i++)
	{
		...
		if ((i == 0) || (usAddr & (macEE_PAGE_SIZE - 1)) == 0)
		{
			...
			*/
			...
			/* 第4步：发送字节地址，24C02只有256字节，因此1个字节就够了，如果是24C04以上，那么此处需要连发多个地址 */
			i2c_ee_SendByte((uint8_t)usAddr);
